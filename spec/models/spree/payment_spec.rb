...
RSpec.describe Spree::Payment, :klarna_api do
  ...
  describe "#refresh!" do
    it "calls the api to update the source" do
      expect(payment.send(:payment_gateway)).to receive(:get_and_update_source).with(payment.source.order_id).and_return(true)
      payment.refresh!
    end
  end
  ...
  describe "#extend_period!" do
    it "calls the api to extend credit authorization period" do
      expect(payment.send(:payment_gateway)).to receive(:extend_period).with(payment.source.order_id).and_return(true)
  ...
  describe "#release!" do
    it "calls the api to release remaining amount" do
      expect(payment.send(:payment_gateway)).to receive(:release).with(payment.source.order_id).and_return(true)
      payment.release!
    end
  end

  describe "#is_klarna?" do
